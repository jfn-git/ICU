...
CQL implementation, but a conservative value-suppression test. Throughout, the
...
training episodes (5 seeds, 95\% CI), computed exactly against $V^\star$ rather than
...
larger for the stronger baselines: exact $\dist$ gains of $+0.014$ (Sarsa), $+0.014$
(Q-learning), $+0.026$ (DQN), and $+0.027$ (PPO).\footnote{Policies are from the
...
baselines.} The model-based endpoint is itself a cautionary example of why we
evaluate exactly: the naive admissibility-aware model-based agent (AA-MBRL, P1$+$P2)
attains a near-oracle \emph{rollout} return ($0.874$) yet an exact $J$ of only
$0.803$ (gap $0.072$) --- no better than masked $Q$-learning --- because its
optimism-driven policy is over-valued under mean-imputed dynamics. Only once an
expert (KL-to-clinician) prior is added does the model-based agent (AA-MBRL-KL) reach
exact $J{=}0.874$ (gap $0.001$). We read this not as ``winning'' but as confirmation
that the gap is an artifact of mean-imputation that the shipped model structure, used
carefully, can close; on a $716$-state tabular MDP a model-based agent reaching
...
rate. Exact $J(\pi)$ is shown as mean${\pm}$std over 5 seeds. Row groups denote the
structural \emph{priors} added to the base learner: \emph{P1} $=$ admissibility
prior (support mask; the AA-$\ast$ methods), \emph{P2} $=$ model prior
(empirical-model planning), and \emph{P4} $=$ expert prior (KL-to-clinician).
Note the model-based contrast: naive AA-MBRL (P1$+$P2) attains a near-oracle
\emph{rollout} return ($0.874$) but an exact $J$ of only $0.803$; only the
expert-regularized AA-MBRL-KL closes the gap exactly. AA-MBRL variants use an
online empirical model and are \emph{not} fair model-free comparisons---they are
breadth showing the gap is an artifact of ignoring support/model structure, not a
headline result.}
\label{tab:merged}
\begin{tabular}{llccc}
\toprule
...
\midrule
$+$P1 mask & AA-Sarsa   & $0.802{\pm}0.003$ & 0.073 & 0.00 \\
          & AA-Q        & $0.801{\pm}0.003$ & 0.074 & 0.00 \\
          & AA-DQN      & $0.826{\pm}0.003$ & 0.050 & 0.00 \\
          & AA-PPO      & $0.869{\pm}0.001$ & 0.006 & 0.00 \\
\midrule
$+$P1$+$P2 & AA-MBRL (UCB) & $0.803{\pm}0.002$ & 0.072 & 0.00 \\
\midrule
$+$P1$+$P2$+$P4 & AA-MBRL-KL & $0.874{\pm}0.000$ & 0.001 & 0.00 \\
\bottomrule
\end{tabular}
\end{table}
...
data (5 seeds, 95\% CI; exact evaluation against $V^\star$).}
\label{fig:offline}
\end{figure}
...
\texttt{terminate} strategy; ICU-Sepsis has no dead-end states \citep{fatemi2021deadends} under the exact
