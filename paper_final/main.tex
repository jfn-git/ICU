...
baselines.} At the model-based endpoint, the admissibility-aware model-based agent
(AA-MBRL) reaches exact $J{=}0.874$ (gap $0.001$). We read this not as ``winning''
but as confirmation that the gap is an artifact of mean-imputation: using the
benchmark's own model structure together with support handling closes it, and on a
$716$-state tabular MDP a model-based agent reaching $V^\star$ is expected
\citep{auer2008ucrl,osband2013psrl}.
...
prior (support mask; the AA-$\ast$ methods) and \emph{P2} $=$ model prior
(empirical-model planning, AA-MBRL). The model-based AA-MBRL uses an online
empirical model and is \emph{not} a fair model-free comparison---it is breadth
showing the gap is an artifact of ignoring support/model structure, not a headline
result.}
\label{tab:merged}
\begin{tabular}{llccc}
\toprule
...
\midrule
$+$P1$+$P2 & AA-MBRL    & $0.874{\pm}0.000$ & 0.001 & 0.00 \\
\bottomrule
\end{tabular}
\end{table}
