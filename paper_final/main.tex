...
\begin{abstract}
ICU-Sepsis exposes a rare opportunity to audit action-support failures relevant
to offline RL: although it is distilled from real ICU trajectories, it ships tabular
...
deployment removes unsupported deployed actions but leaves the learned $Q$-table
...
Methodologically, the mechanism study (\S\ref{sec:failure}--\S\ref{sec:remedy})
uses the released MDP as a controlled white-box diagnostic; \S\ref{sec:offline}
adds a fixed finite-data offline test, and behavior-level masking localizes
\emph{where} the failure enters learning rather than altering logged data.
...
\paragraph{Setup.}
The mechanism-diagnosis experiments treat the released tabular MDP as a controlled
diagnostic simulator---online only in the methodological sense of probing its
dynamics, not a deployable offline clinical training protocol.
...
$5{\times}10^5$). DQN and PPO use one-hot state inputs with linear heads
(equivalent to tabular $Q$- and policy-tables) and adopt the ICU-Sepsis paper's
published hyperparameters \citep{choudhary2024icusepsis}; their single-file
CleanRL-style implementations are in the linked code, and we use them only as
breadth checks, not tuned deep-RL baselines. We report means over 5 seeds; shaded bands in line plots show 95\% confidence
...
small training budget: it persists across Sarsa, Q-learning, DQN, and PPO, and Q-learning remains
...
whereas pessimism and masking remain calibrated. In a fixed offline dataset, the
analogous intervention is not to alter past behavior but to encode action support
and keep unsupported actions out of planning, bootstrapping, and policy selection
(via masking, pessimism, or conservative penalties).
...
and leakage $+0.46\to-0.80$). The benchmark clinician policy is inherited as a
reference policy and is not designed as an admissibility-control method.
