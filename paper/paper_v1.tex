...
\begin{figure}[t]\centering
\includegraphics[width=0.72\linewidth]{exact_dist_curve.png}
...
\begin{figure}[t]\centering
\includegraphics[width=0.78\linewidth]{offline_datasize.png}
...
Offline medical RL learns from logged data with uneven action coverage, and how a
pipeline handles data-unsupported actions is a consequential design choice that
survival rate can hide. We make this failure visible and exactly measurable on a
controlled testbed and package a support-aware diagnostic-and-remedy protocol---a
methodological caution, not a clinical recommendation.
...
data-size sweep, expert projection, robustness, dead-ends) are computed from the
shipped ICU-Sepsis dynamics by exact value iteration and reproduce from the released
one-command scripts (metric definitions in \S\ref{sec:method}). The cross-algorithm
and model-based ($5{\times}10^5$) results are obtained by exactly evaluating the
teammate's committed policies against the true dynamics with the released evaluation
script, so every number in Table~\ref{tab:merged} is reproducible.
