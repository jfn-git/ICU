...
Beyond the mean-imputation default, ICU-Sepsis ships per-state structure that
standard agents discard during training---admissibility, an empirical transition
model, severity, and a clinician policy. Our main line uses admissibility and the
empirical model (severity and the clinician prior are supplementary); putting this
discarded structure back is what removes the pathology.
...
\begin{enumerate}[leftmargin=2em,itemsep=2pt,topsep=2pt]
  ...
  As supporting evidence, the same support-aware improvement pattern (support
  penalty $\to$ CQL conservatism $\to$ admissibility masking $\to$ model-based
  endpoint AA-MBRL) appears across Sarsa/Q/DQN/PPO and in a finite-data offline
  empirical-MDP regime,
...
and study a spectrum of remedies. Our mechanism study (\S\ref{sec:mechanism}) uses
tabular $Q:\mathcal{S}\times\mathcal{A}\to\mathbb{R}$ updates to isolate \emph{where}
support is enforced rather than the effect of function approximation; we then test
whether the same pattern appears in broader agent classes (DQN, PPO) and a
model-based learner (\S\ref{sec:remedy}).
...
u_{\Adm(s)})$ with $u_{\Adm(s)}$ uniform on $\Adm(s)$ --- a value-level conservatism. This is not a full
CQL implementation but a CQL-inspired tabular conservative regularizer, testing
whether soft value suppression can substitute for hard support masking.
...
deployed unsupported-action rate, the fraction of non-terminal states (counted
uniformly) where the greedy policy picks $a\notin\Adm(s)$; \emph{Q-value leakage},
...
$1.0{\to}0.05$ over $10^4$ episodes; cross-algorithm and model-based results
(Table~\ref{tab:merged}) use $5{\times}10^5$. Vanilla Q-learning stays at the
baseline level at both budgets ($J{=}0.785$ at $5{\times}10^4$, $0.787$ exact at
$5{\times}10^5$), so the failure is not under-training. We report mean $\pm$ std
(per-seed scatter in the appendix) and rest conclusions on large-support
diagnostics, not small survival differences.

\paragraph{Baselines reproduce the benchmark.}
Our exact $d_0$-weighted values match the published Random/Expert/Optimal survival
rates ($0.780$/$0.782$/$0.875$; the Reference rows of Table~\ref{tab:merged}),
confirming the ruler is calibrated; these values live in a narrow $0.78$--$0.875$
band, motivating the more sensitive $\dist$.
...
$J{=}0.806$, $\dist{=}0.069$ (at $5{\times}10^4$; cf.\ the exact $5{\times}10^5$ AA-Q policy, $J{=}0.801$, Table~\ref{tab:merged}). The failure is not specific to Q-learning or to a
...
ablation}, behavior masking matches the full mask and is the only component that clears Q-value leakage.}
\label{tab:ablation}
\begin{tabular}{lcccl}
\toprule
...
% --- HERO FIGURE: highest priority. 0.90 width (nudged up from the 0.85 trim now
% that float placement freed room). Do not let the ablation table crowd it out. ---
\begin{figure}[tb]\centering
\includegraphics[width=0.90\linewidth]{component_ablation.png}
...
teammate runs ($5{\times}10^5$ episodes, 5 seeds, committed in the released repo),
re-evaluated against the true dynamics ($V^\pi$ by value iteration), so every trained
value is $\le V^\star$---unlike the benchmark's rollout metric, under which some
near-optimal cells spuriously exceed it. ``Deploy unsupp.'' is the deployed
inadmissible-action rate. The model-based endpoint AA-MBRL uses an online empirical
model and is \emph{not} a fair model-free comparison---supporting breadth, not a
headline result.}
\label{tab:merged}
\begin{tabular}{llccc}
\toprule
...
the true $V^\star$ (Fig.~\ref{fig:offline}). The two differ in support: pessimism
\emph{estimates} it from the $N$ samples (a count-based lower-confidence bonus over
observed actions), whereas masking uses the benchmark's \emph{oracle} admissible set
$\Adm(s)$---so masking isolates the value of known support labels, pessimism the
realistic estimate-from-data case. Naive overestimates its own value
...
\begin{figure}[tb]\centering
\includegraphics[width=1.0\linewidth]{offline_datasize.png}
...
chosen when inadmissible actions behave like random admissible ones. Survival rate
alone can therefore obscure the \emph{mechanism}: exact value gaps and support
diagnostics ($\dist$, deployed-unsupported rate, $Q$-leakage) are needed to explain
\emph{why} a policy attains a given survival rate.

\paragraph{Why a known-dynamics benchmark.}
Exact $V^\star$ is a \emph{measurement instrument}, not an optimization claim. The
OOD-action pathology and the overestimation it induces is a well-recognized property
of offline RL \citep{kumar2020cql,kumar2019bear}; ICU-Sepsis simply lets us measure
it cleanly, sidestepping the noisy off-policy estimation real cohorts force
\citep{gottesman2019guidelines}. We claim not that the benchmark is more realistic
than real data, only that it is a particularly clean ruler.
...
Offline medical RL learns from logged data with uneven action coverage, where the
handling of data-unsupported actions is a consequential design choice that survival
rate can hide. We make this failure exactly measurable on a controlled testbed and
package a support-aware diagnostic-and-remedy protocol---a methodological caution,
...
evaluations of committed saved policies against the true dynamics with the same script.
...
\label{app:figs}
This appendix collects the penalty-sweep remedy figure (Fig.~\ref{fig:spectrum}) and
the figures for the supplementary analyses of \S\ref{sec:supp} (expert projection,
robustness, dead-end structure).
