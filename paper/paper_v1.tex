...
maskable policies \todo{cite Stable-Baselines3 MaskablePPO (Raffin et al.)}. We
...
device \citep{kumar2019bear} \todo{cite Wu/Nair behavior-regularized offline RL}.
...
\label{app:figs}
\todo{Overview schematic (Fig.~1, to be drawn; see core\_docs/15 \S4): show
intuitively how mean imputation makes an unsupported action ``learnable without
penalty,'' so a model-free agent adopts it.}

\todo{Teammate learning curves (sample efficiency; PNG to be supplied) and the
AA-MBRL-Shaped/-KL/-Full variants (marked \emph{in progress} in the teammate draft;
omitted until reproduced).}

\todo{$5{\times}10^5$-episode confirmation run for vanilla / behavior-mask /
full-mask (App.), to show the component-ablation ordering and leakage conclusions
are identical at the larger budget; bridged for now by vanilla Q $=0.785$ at both
budgets (Table~\ref{tab:merged}).}
